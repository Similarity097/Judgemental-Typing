...
class typing:
	...
	def result(self):
		if not self.end:
			...
			pygame.draw.rect(self.screen, (191, 58, 238), (515, 565, 170, 80), 3)
	...
	def run2(self):
		...
		while (self.running):
			...
			self.text_display(self.screen, self.input_text, 600, 372.5, 35, self.white)
				...
				if event.type == QUIT:
					...
					if self.active and not self.end and not self.type:
						if event.key == pygame.K_RETURN:
							self.result()
							self.text_display(self.screen, self.results, 600, 500, 30, self.white)
							self.end = True
